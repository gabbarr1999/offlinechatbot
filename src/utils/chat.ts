import { applyTemplate, Templates, Conversation } from 'chat-formatter';
...
export function convertToChatMessages(
  ...
  return messages
    .filter(message =>
      ...
      (message.type === 'image' && message.imageUri !== undefined)
    )
    .map(message => {
      if (message.type === 'image') {
        return {
          content: {
            type: 'image',
            imageUri: (message as MessageType.Image).imageUri
          },
...
export async function applyChatTemplate(
  ...
  try {
    ...
    const conversationMessages = convertToConversation(messages);

    if (modelChatTemplate?.chatTemplate) {
      formattedChat = applyTemplate(conversationMessages, {
...
function convertToConversation(messages: ChatMessage[]): { role: string; content: string }[] {
  return messages.map(msg => ({
    role: msg.role,
    content: typeof msg.content === 'string' ? msg.content : `[Image: ${msg.content.imageUri}]`
  }));
}
